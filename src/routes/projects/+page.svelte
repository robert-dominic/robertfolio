<script lang="ts">
	...
	const totalPages = $derived(Math.max(1, Math.ceil(filteredProjects.length / perPage)));
	const safePage = $derived(Math.min(Math.max(currentPage, 1), totalPages));
	const paginatedProjects = $derived(filteredProjects.slice((safePage - 1) * perPage, safePage * perPage));
...
<svelte:head>
	...
	<meta property="og:description" content="A broader look at Robert's product work across frontend systems, dashboards, platforms, and editorial experiences." />
	...
	<meta name="twitter:description" content="A broader look at Robert's product work across frontend systems, dashboards, platforms, and editorial experiences." />
</svelte:head>
...
<main id="main-content" class="section-shell pb-20 pt-12 sm:pt-16">
	...
	<div class="flex items-center justify-between gap-4">
		...
		<div class="flex items-center gap-2.5">
			<label for="type-filter" class="caption-text hidden sm:block shrink-0">Filter by</label>
			<div class="relative">
				<select
					id="type-filter"
					value={activeType}
				...
				>
					<option value="all" style="background: var(--color-bg); color: var(--color-text);">All</option>
					<option value="Frontend" style="background: var(--color-bg); color: var(--color-text);">Frontend</option>
					<option value="Full Stack" style="background: var(--color-bg); color: var(--color-text);">Full Stack</option>
				</select>
				<svg
	...
	<p class="mt-5 text-measure-tight">
		A broader look at Robert's product work across frontend systems, dashboards, platforms, and
	...
	<div class="mt-10 flex items-center gap-3">
		...
		/>
		<p class="text-sm text-[var(--color-text-soft)]">Page {safePage} of {totalPages}</p>
		<SparkleButton
			label="Next"
			variant="secondary"
